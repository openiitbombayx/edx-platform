...
from functools import partial #IITBombayX TickCrossProblem 6th Aug 2015
...
class CapaMixin(CapaFields):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #IITBombayX TickCrossProblem 6th Aug 2015 starts
        #log.info("***************************************** node")
        #log.info(type(self))
        if isinstance(self, dict):
            geta = self.get
        else:
            geta = partial(getattr, self)

        geta=partial(getattr, self)
        #log.info(str(geta('due', None)))
        #log.info(str(geta('graded',None)))
        self.due_graded = geta('graded', None)
        #self.due_timed = self.is_past_due()
        self.runtime.set('dgrade', geta('graded',None))
		#IITBombayX TickCrossProblem 6th Aug 2015 ends
		...
		#IITBombayX TickCrossProblem 6th Aug 2015 starts
        self.due_timed = self.is_past_due()          #
        self.due_answer = self.past_due_answered()   #
        self.now_attempt = self.now_attempts()    #
        self.last_attempt =  geta('attempts', 0)  #
		#IITBombayX TickCrossProblem 6th Aug 2015 ends
    ...
    def set_state_from_lcp(self):
		...
		#IITBombayX TickCrossProblem 6th Aug 2015 starts
        self.due_graded = lcp_state['due_graded']     #
        self.now_attempt = lcp_state['now_attempt']   #
        self.last_attempt = lcp_state['last_attempt'] #
        self.due_timed = lcp_state['due_timed']       #
        self.due_answer = lcp_state['due_answer']     #
		#IITBombayX TickCrossProblem 6th Aug 2015 ends
    ...
    def get_problem_html(self, encapsulate=True):
        ...
        context = {
			...
			#IITBombayX TickCrossProblem 6th Aug 2015 starts
            'flagnoy': self.is_past_due(),
            'pname' : self.location.name,
            'last_attempt': self.last_attempt,
            'due_graded': self.due_graded,
            'due_answer':self.due_answer,
            'due_timed': self.due_timed,
            'rtime': self.runtime.dgrade, 
			#IITBombayX TickCrossProblem 6th Aug 2015 ends
   		    'demand_hint_possible': demand_hint_possible
        }
		#IITBombayX TickCrossProblem 6th Aug 2015 starts
		#log.info("answer_available1:")
        answer_available=self.answer_available()
        #log.info(str(answer_available))
		#IITBombayX TickCrossProblem 6th Aug 2015 ends
	...
	#IITBombayX TickCrossProblem 6th Aug 2015 starts
    def now_attempts(self):
        """
        Number of attempts 
        """
        #return self.lcp.now_attempts()
        if self.attempts is not None:
            return self.attempts
        else:
            return 0

    def past_due_answered(self):
        """
    ...
    def check_problem(self, data, override_time=False):
        ...
        try:
            ...
            #IITBombayX TickCrossProblem 6th Aug 2015
            self.now_attempt = self.now_attempt + 1 #
            self.lcp.done = True
            self.set_state_from_lcp()
