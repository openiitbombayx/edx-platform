...
#Bharat MOOCs 17th Sept 2014- display count of courses, faculty and certificates
...
#Bharat MOOCs 9th Sept 2014
from courseware.models import StudentModule, StudentModuleHistory, Organization
...
#Bharat MOOCs 8th Sept 2014
#Added CourseAccessRol
from student.models import  CourseAccessRole

#Bharat Moocs 10th Sept count number of generated certificate
...
#Bharat MOOCs 23rd Sept Pagination functionality added
from django.core.paginator import Paginator,EmptyPage, PageNotAnInteger

#Bharat Moocs 17th dec redirection to language specific partner page
from django.utils.translation import ugettext as _
from django.utils import translation
# Bharat Mooc  24 Dec Added for counting Total instructor and staff in partners page
...
#Bharat Mooc 
#5 september
#For browsing all the list of universities
@ensure_csrf_cookie
def browse_organization(request):
        org=Organization.objects.all()
        #staff=.objects.all().filter(is_staff=1)
        #Added 10th sept staff and generatedcertificate
        #staff=CourseCreator.objects.filter(state ='granted').count()
        # Bharat Mooc  31 Dec Added for counting Total instructor and staff in partners page
        #staff=CourseAccessRole.objects.all().filter(Q(role='instructor') | Q(role = 'staff')).count()
        #IITBombayX 3rd Jan 2015 number of faculty members is the count of unique user ids in student_courseaccessrole table
        staff=CourseAccessRole.objects.all().values('user').distinct().count()
        generatedcertificate=GeneratedCertificate.objects.all().count()
        courses=len(get_courses(request.user))
        #Bharat Moocs Added on 23 Sep for language option
        language_options = DarkLangConfig.current().released_languages_list
        context = {'org':org, 'staff':staff,'generatedcertificate':generatedcertificate,'courses':courses,'language_options':language_options}
        cur_language = translation.get_language()
	# IITBX 11-APR-2016:code changes render_to_response('templates/'+... )
        #return render_to_response('static_templates/'+cur_language + '/'+ 'browse_org.html', context)
        return render_to_response('templates/'+cur_language + '/'+ 'browse_org.html', context)
...
def course_info(request, course_id):
    ...
    with modulestore().bulk_operations(course_key):
        ...
        context = {
            ...
            'masquerade': masquerade,
            'studio_url': studio_url,
#	This variable was there in Aspen release but not there in Cypress
...
#Ashutosh Bhargave
def organization_about(request,org_id):
    """
    Display the Organization's about page.
    Assumes the org_id is in a valid format.
    """
...
# 6th Apr 2016
    ...
    universities=dictfilt(universities, wanted_keys)
    courseDre=universities.get(org_id)
    courseDe=[]
    if (courseDre != None):
     for course in courseDre:
      ...
      tmp.append(start)
      course_id=course.id
     #IITBombayX changes for supporting dynamic url
      course_url=course.id.to_deprecated_string()
      course=course_id.course
      org=course_id.org
#Bharat MOOCs 19th Sept 2014 Added for creating href in order to open course in course listing on home page
      run=course_id.run
      tmp.append(course)
      tmp.append(org)
      tmp.append(run)
...
# Bharat MOOCs 19th Sept 2014 Language selection box added
    context={"coursePg":coursePg,"language_options":language_options,"org":org}

    #context={"courseDe":courseDe,'org': org}

#IITBombayX May 2016: For custom UI using themes; courselist.html should come from themes
    return render_to_response('courselist.html',context)


# Bharat_MOOCs
# Added for displaying courses hosted by a particular university
# 28th August 2014
def get_courses_based_on_university(request, domain=None):
    ...
    instobj=CourseAccessRole.objects.filter(role ='instructor')
    #log.info("Anand")
    #log.info(dir(instobj))
    #for i in instobj:
     # log.info(i.user_id)
    for course in visible_courses:
        universities[course.org].append(course)
        #universities=[elem for elem in universities if course.org== 'IITBY']
    universities=dictfilt(universities, wanted_keys)

    courseDre=universities.get('IITBombayX')
    courseDe=[]
    for course in courseDre:
      tmp=[]
      title=get_course_about_section(course,'title')
      ...
      #IITBombayX : To support dynamic link based on split and draft courses
      course_url=course.id.to_deprecated_string()
...
# Bharat MOOCs 19th Sept 2014 Language selection box added
    ...
    return render_to_response('static_templates/courselist.html',context)
# Bharat_MOOCs ends
# Added for displaying courses hosted by a particular university
# 28th August 2014
